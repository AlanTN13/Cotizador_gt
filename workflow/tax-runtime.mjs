// Generated from lib/courier/tax-resolver.ts; run node workflow/build-workflow.mjs.
import dieTeData from "../data/tax-die-te.json" with { type: "json" };
import estimationPolicy from "../data/tax-estimation-policy.json" with { type: "json" };
import vatData from "../data/tax-vat.json" with { type: "json" };
export const TAX_RESOLVER_VERSION = "tax-resolver-1.1.0";
export const TAX_SCOPE = "DIE_TE_IVA_REFERENCIAL_V1";
export const taxDataset = { duty: dieTeData, vat: vatData };
...
const validRate = (rate) => Number.isFinite(rate) && rate >= 0 && rate <= 1;
function usable(source, now) {
...
export function resolveTaxes(input, productIndex, now = new Date(), data = taxDataset, profileEstimate) {
    ...
    if (dutyReviewDue || vatReviewDue)
        result.warnings.push("TAX_SOURCE_REVIEW_DUE");
    ...
    if (matches.length) {
        ...
        if (matches.some(([, row]) => !validRate(row.duty) || !validRate(row.statistical) ||
            ...
            result.reasons.push("DIE_TE_AMBIGUOUS_OR_INVALID");
            return result;
        }
        ...
        if (!usableProfile || usableProfile.duty === null || !validRate(usableProfile.duty)) {
            result.reasons.push("NO_MINIMUM_TAX_BASIS");
            return result;
        }
        ...
        if (profileTe != null && !validRate(profileTe)) {
            result.reasons.push("PROFILE_RATE_INVALID");
            return result;
        }
        if (profileTe == null && (+now < Date.parse(general.validFrom) || +now >= Date.parse(general.validUntil))) {
            result.reasons.push("GENERAL_TE_OUTSIDE_LEGAL_PERIOD");
            return result;
        }
        ...
        estimate("duty", "PROFILE_RATE", profileSource, "DIE de referencia del perfil para la misma posición probable; sin coincidencia en snapshot.");
        estimate("statistical", profileTe != null ? "PROFILE_RATE" : "GENERAL_RULE", profileTe != null ? profileSource : general.source, profileTe != null ? "TE específica del perfil para la misma posición probable." : general.basis);
    }
    ...
    if (!vatPositions.length)
        vatPositions.push(position);
    const selected = [];
    for (const sim of vatPositions) {
        ...
        if (rules.some((rule) => !validRate(rule.vat) || !rule.basis || !rule.sourceIds.length ||
            ...
            result.reasons.push("VAT_RULE_INVALID");
            return result;
        }
        if (rules.some((rule) => rule.vat !== rules[0].vat)) {
            result.reasons.push("VAT_AMBIGUOUS");
            return result;
        }
        if (rules.length > 1)
            result.warnings.push("VAT_EQUIVALENT_RULES");
        if (rules.length)
            selected.push(rules[0]);
            ...
            if (+now < Date.parse(general.validFrom)) {
                result.reasons.push("GENERAL_VAT_NOT_AVAILABLE");
                return result;
            }
